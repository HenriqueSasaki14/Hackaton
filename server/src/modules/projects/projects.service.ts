...
export async function updateProject(projectId: string, userId: string, data: {
  ...
  if (project.status === 'PUBLISHED') {
    throw Object.assign(new Error('Projetos publicados não podem ser editados'), { status: 400 })
  }
...
export async function publishProject(projectId: string, userId: string) {
  ...
  if (project.user_id !== userId) throw Object.assign(new Error('Sem permissão'), { status: 403 })
  if (project.status === 'PUBLISHED') {
    throw Object.assign(new Error('Projeto já está publicado'), { status: 400 })
  }

  const trailDone = await isTrailCompleted(userId, project.trail_id)
  if (!trailDone) {
    throw Object.assign(
      new Error('Conclua 100% da trilha antes de publicar o projeto'),
      { status: 403 }
    )
  }
...
export async function toggleLike(projectId: string, userId: string) {
  ...
  if (!project) throw Object.assign(new Error('Projeto não encontrado'), { status: 404 })
  if (project.status !== 'PUBLISHED') {
    throw Object.assign(new Error('Só é possível curtir projetos publicados'), { status: 400 })
  }
